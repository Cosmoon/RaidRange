...
function Comms:Broadcast(elapsed)
  ...
  local maxShow = (RaidRangeDB and RaidRangeDB.threshold) or 10
  local maxSend = maxShow + 5
  local bytes = {}

  bytes[#bytes + 1] = string.char(IsInRaid() and 1 or 0) -- header
  ...
  if count > 0 then
    ...
    if self.db and self.db.debug then
      print(("|cff33ff99RaidRange|r sent %d entries (%s)"):format(count, chan))
    end
  end
end
...
function Comms:OnMessage(sender, msg)
  ...
  if self.db and self.db.debug then
    ...
    print(("|cff33ff99RaidRange|r recv %d from %s"):format(c, sender))
  end
end
